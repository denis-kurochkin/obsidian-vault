...
```
TLP  — Transaction Layer PacketDLLP — Data Link Layer Packet
```
...
```
TLP защищается на конкретном linkreceiver проверяет, что packet не поврежден
```
...
```
Physical link trainedData Link initialization completedConfiguration/enumeration completed
```
...
```
User DMA logic    |    vTransaction Layer:    формирует Memory Write TLP    address + byte count + payload    |    vData Link Layer:    добавляет link-level reliability context    sequence / LCRC / replay handling    |    vPhysical Layer:    передает packet через lanes    |    vRoot Complex receives
```
