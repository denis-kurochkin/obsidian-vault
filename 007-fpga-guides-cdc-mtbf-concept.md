...
```
неправильно принятый flagпотерянный eventлишний eventнеправильный переход state machineложный старт операцииошибка full/empty внутри FIFOредкий зависон control logic
```
...
```
если FF1 стал metastable,у него есть почти один период clk_dst,чтобы стабилизироваться до захвата FF2
```
...
```
одна часть схемы увидела 0другая часть схемы увидела 1
```
...
```
FF1 получает async inputFF2 получает уже задержанную версию FF1
```
...
```
100 MHz -> 10 ns250 MHz -> 4 ns500 MHz -> 2 ns
```
...
```
destination clock быстрыйсигнал часто переключаетсяустройство должно работать годамиотказ дорогойсигнал влияет на critical state machinecrossing используется во многих экземплярахметодология проекта требует 3-stage synchronizers
```
...
```
увеличивается latencyусложняется control timingможно нарушить protocolможет увеличиться resource usageсобытия могут приходить быстрее, чем chain успевает их отразить
```
