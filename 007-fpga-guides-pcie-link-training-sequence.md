...
lane polarity / reversal-related information;
переход к следующим substates.
```
...
```
не все lanes подключены;
часть lanes не работает;
lane mapping ошибка;
host/root port ограничен;
slot bifurcation;
retimer/topology;
board issue;
IP configured capability отличается от ожидания.
```
...
```
какая физическая lane соответствует logical lane 0;
есть ли lane reversal;
какие lanes участвуют в link.
```
...
```
FPGA lane 0 -> connector lane 3
FPGA lane 1 -> connector lane 2
FPGA lane 2 -> connector lane 1
FPGA lane 3 -> connector lane 0
```
...
```
LTSSM near L0 / LinkUp — хорошо,
но для TLP traffic важен завершенный Data Link initialization.
```
...
```
сначала установить базовую связь;
потом попробовать повысить скорость.
```
...
```
после начального L0 для перехода на более высокую speed;
при retraining;
при ошибках link;
после потери lock;
при попытке изменить width/speed;
во время equalization.
```
...
```
link может успешно работать на Gen1,
но не проходить Gen3/Gen4 equalization.
```
