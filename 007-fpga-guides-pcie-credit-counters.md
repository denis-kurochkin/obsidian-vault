...
```
limit       — сколько credits выдано / максимальная граница;consumed    — сколько credits уже потрачено на отправленные TLP;available   — сколько еще можно отправить.
```
...
```
LTSSM = L0Gen3/Gen4 speed correctwidth correctlspci видит устройствоBAR работает
```
...
```
user logic:    tvalid = 1PCIe core:    tready = 0
```
...
```
reset;link not ready;internal buffer full;configuration not complete;protocol block busy;user interface throttling.
```
...
```
cfg_fc_pd_scalecfg_fc_nph_scalecfg_fc_npd_scalecfg_fc_cplh_scalecfg_fc_cpld_scale
```
...
```
cfg_fc_selcfg_fc_phcfg_fc_pdcfg_fc_nphcfg_fc_npdcfg_fc_cplhcfg_fc_cpldRQ/CC/CQ/RC valid/readyDMA request valid/readyDMA payload sizecurrent_speednegotiated_widthlink_up / dl_upreset/user_ready
```
...
```
traffic данного типа почти не идет;receiver имеет много buffer space;этот pool не является bottleneck;значение масштабировано;pool фактически неограничен для данного сценария;ты смотришь не тот cfg_fc_sel режим.
```
...
```
какой именно pool около нуля;какой traffic идет;есть ли tready drops;есть ли gaps между TLP;не является ли это нормальным при burst;не ограничивает ли другой ресурс.
```
...
```
NPH — чтобы отправить requests;CplH/CplD — чтобы принять returning completions.
```
