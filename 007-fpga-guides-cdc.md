...
```
01или временно находиться в неопределенном аналоговом состоянии
```
...
```
always @(posedge clk_b)    sync_signal <= async_signal;
```
...
```
1-bit level signal1-bit pulsemulti-bit data busvalid/data handshakestreaming dataFIFO crossingreset crossing
```
