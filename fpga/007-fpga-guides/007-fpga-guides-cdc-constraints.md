...
```
set_clock_groupsset_false_pathset_max_delay -datapath_only
```

Дополнительно для анализа:

```
report_clock_interactionreport_cdcreport_timing_summary
```
...
```
set_max_delay -datapath_only <value> \    -from <source_objects> \    -to   <destination_objects>
```
...
```
Gray-code bus crossingнесколько independent bits с требованием bounded skewCDC structures, где важна ограниченная задержка до synchronizer
```
...
```
если используешь XPM_CDC,не надо автоматически ставить широкий set_clock_groups,не проверив, как это влияет на XPM constraints.
```
