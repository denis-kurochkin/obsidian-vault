...
Power / REFCLK / PERST#
        |
FPGA configuration / PCIe IP ready
        |
GT / PHY ready
        |
LTSSM -> L0
...
```
Каждая фаза проверяет свой уровень.
Не переходи к debug следующего уровня, пока предыдущий не доказан.
```

Практическая формула:

```
Нет L0       -> debug physical/link training.
Есть L0, нет lspci -> debug enumeration/config timing.
Есть lspci, нет BAR access -> debug command/BAR/TLP/user logic.
Есть BAR, нет DMA -> debug Bus Master/descriptor/DMA path.
Есть DMA, плохой throughput -> debug credits/MPS/MRRS/buffering/performance.
```